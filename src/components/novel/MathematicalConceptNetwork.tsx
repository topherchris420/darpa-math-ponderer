...
const createConceptNodes = (): ConceptNode[] => [
  // Topology
...
export const MathematicalConceptNetwork: React.FC<MathematicalConceptNetworkProps> = ({
  ...
  // Nodes live in a ref: the physics loop mutates positions every frame, and
  // strength updates arrive faster than a state-driven effect could keep up
  // with without restarting the loop.
  const nodesRef = useRef<ConceptNode[]>(createConceptNodes());
  ...
  useEffect(() => {
    ...
    nodesRef.current.forEach(node => {
      ...
      node.strength = Math.max(0, Math.min(1, newStrength));
    });
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Draw legend
      (Object.keys(CATEGORY_COLORS) as Array<keyof typeof CATEGORY_COLORS>).forEach((category, index) => {
        const x = 20;
        const y = 20 + index * 25;
